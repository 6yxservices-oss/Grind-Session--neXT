    ...
    <div className="space-y-6">
      <div>
        ...
        <div className="flex items-center gap-4">
          <div className="w-16 h-16 rounded-xl bg-eybl-accent/10 flex items-center justify-center text-eybl-accent font-bold text-2xl">
            {team.name.slice(0, 2).toUpperCase()}
          </div>
          <div>
            <h1 className="text-3xl font-bold">{team.name}</h1>
            <p className="text-gray-400">{team.program} &middot; {team.city}, {team.state}</p>
          </div>
        </div>
      </div>
        ...
        {team.players.length > 0 ? (
          <table className="w-full">
            <thead className="border-b border-gray-800">
              <tr>
                {["Player", "Position", "Class", "Height", "Weight", "Rating"].map((h) => (
              ...
              {team.players.map((p) => (
                <tr key={p.id} className="hover:bg-white/5">
                  <td className="table-cell">
                    <Link href={`/players/${p.id}`} className="text-white hover:text-eybl-accent font-medium">
                  ...
                  <td className="table-cell">{p.height || "—"}</td>
                  <td className="table-cell">{p.weight ? `${p.weight} lbs` : "—"}</td>
